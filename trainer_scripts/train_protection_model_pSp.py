...
if __name__ == '__main__':
    ...
    model_ckpt_pth = os.path.join(PSP_ATTACK_BASE_DIR, net_noise)
    ...
    logger_train = Logger(model_name=architecture_type, data_name=dataset_type, log_path=os.path.join(TF_BASE_DIR, 'attacks/pSp/', net_noise, 'train', optimizer.__name__ + "_perturb" + str(perturb_wt) + "_batch" + str(batch_size) + "_lr_" + str(optim_args['lr']) + "_" + str(loss_type) + "loss"))
    logger_val = Logger(model_name=architecture_type, data_name=dataset_type, log_path=os.path.join(TF_BASE_DIR, 'attacks/pSp/', net_noise, 'val', optimizer.__name__ + "_perturb" + str(perturb_wt) + "_batch" + str(batch_size) + "_lr_" + str(optim_args['lr']) + "_" + str(loss_type) + "loss"))
    ...
    for epoch in range(1, n_epochs):  # outer loop for different epochs;
        ...
        for i, data in enumerate(tqdm(train_loader, desc='')):  # inner loop within one epoch
            ...
            y_target = create_target_img(batch_size=batch_size, img_transform=adv_img_transform)  # Color Coded output to produce
