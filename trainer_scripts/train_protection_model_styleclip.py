...
if __name__ == '__main__':
    ...
    for epoch in range(n_epochs):  # outer loop for different epochs;
        ...
        if val_current_loss['full'] < best_loss:
            save_filename_model = 'styleClip_protection_%s_%sperturb.pth' % (net_noise, perturb_wt)
        ...
        save_filename_model = 'styleClip_protection_%s_%sperturb_latest.pth' % (net_noise, perturb_wt)
        save_path = os.path.join(model_ckpt_pth, save_filename_model)
        torch.save({"protection_net": protection_model.state_dict(), "global_noise": adv_noise.detach(), "perturb_wt": perturb_wt}, save_path)
